...
public sealed class GoogleCallbackHandler
{
    ...
    public async Task<GoogleCallbackResponse> HandleAsync(
        Guid applicationId,
        string code,
        string state,
        string redirectUri,
        CancellationToken ct)
    {
        ...
        else
        {
            ...
            if (existingUser != null)
            {
                ...
                _logger.LogInformation(
                    "Linked Google account to existing user {UserId} in application {AppId}",
            ...
            else
            {
                ...
                _logger.LogInformation(
                    "Created new user {UserId} via Google OAuth in application {AppId}",
                    user.Id, applicationId);
            }
        }
        ...
        return new GoogleCallbackResponse(tokens, isNewUser);
    }
}
...
public static class GoogleCallback
{
    ...
    {
        app.MapGet($"{prefix}/apps/{{appId:guid}}/auth/google/callback", async (
                Guid appId,
                string code,
                string state,
                string redirectUri,
                ...
                CancellationToken ct) =>
            {
                ...
                return Results.Ok(ApiResponse<GoogleCallbackResponse>.Ok(response));
            })
