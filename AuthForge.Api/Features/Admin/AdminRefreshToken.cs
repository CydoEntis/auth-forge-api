...
public class AdminRefreshTokenHandler
{
    ...
    private readonly ILogger<AdminRefreshTokenHandler> _logger;

    public AdminRefreshTokenHandler(
        AppDbContext context,
        IJwtService jwtService,
        ILogger<AdminRefreshTokenHandler> logger)
    {
        _context = context;
    ...
    public async Task<RefreshTokenResponse> HandleAsync(RefreshTokenRequest request, CancellationToken ct)
    {
        ...
        _logger.LogInformation("Token refreshed for admin {AdminId}", storedToken.AdminId);

        return new RefreshTokenResponse(newTokens);
    }
}

public static class AdminRefreshToken
{
    public static void MapEndpoints(WebApplication app, string prefix = "/api/v1")
    {
        app.MapPost($"{prefix}/admin/refresh", async (
                RefreshTokenRequest request,
                AdminRefreshTokenHandler handler,
