...
public class LogUserRegisteredHandlerTests
{
    private readonly Mock<IAuditLogRepository> _auditLogRepositoryMock;
    private readonly Mock<IUnitOfWork> _unitOfWorkMock;
    private readonly Mock<ICurrentUserService> _currentUserServiceMock;
    private readonly LogUserRegisteredHandler _handler;

    public LogUserRegisteredHandlerTests()
    {
        _auditLogRepositoryMock = new Mock<IAuditLogRepository>();
        _unitOfWorkMock = new Mock<IUnitOfWork>();
        _currentUserServiceMock = new Mock<ICurrentUserService>();

        _handler = new LogUserRegisteredHandler(
    ...
    public async Task Handle_WithValidEvent_ShouldCreateAuditLog()
    {
        ...
        var domainEvent = new EndUserRegisteredDomainEvent(
            userId,
            applicationId,
            email,
            "John",
    ...
    public async Task Handle_ShouldSerializeEventDetailsCorrectly()
    {
        ...
        capturedAuditLog!.Details.Should().Contain(firstName);
        capturedAuditLog!.Details.Should().Contain(lastName);
        capturedAuditLog!.EventType.Should().Be(AuditEventConstants.UserRegistered);
    }
    ...
    [Fact]
    public async Task Handle_ShouldCaptureUserNames()
    {
        // ARRANGE
        var applicationId = ApplicationId.Create(Guid.NewGuid());
        var email = Email.Create("user@example.com");
        ...
        // ASSERT
        capturedAuditLog!.Details.Should().Contain($"\"firstName\":\"{firstName}\"");
        capturedAuditLog!.Details.Should().Contain($"\"lastName\":\"{lastName}\"");
    }
}
