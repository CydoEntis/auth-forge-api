...
public class LogUserManuallyLockedHandlerTests
{
    private readonly Mock<IAuditLogRepository> _auditLogRepositoryMock;
    private readonly Mock<IEndUserRepository> _endUserRepositoryMock;
    private readonly Mock<IUnitOfWork> _unitOfWorkMock;
    private readonly LogUserManuallyLockedHandler _handler;

    public LogUserManuallyLockedHandlerTests()
    {
        _auditLogRepositoryMock = new Mock<IAuditLogRepository>();
        _endUserRepositoryMock = new Mock<IEndUserRepository>();
        _unitOfWorkMock = new Mock<IUnitOfWork>();

        _handler = new LogUserManuallyLockedHandler(
    ...
    public async Task Handle_WithValidEvent_ShouldCreateAuditLog()
    {
        ...
        var lockedUntil = DateTime.UtcNow.AddHours(24);
        ...
        var domainEvent = new EndUserManuallyLockedDomainEvent(userId, applicationId, lockedUntil);
    ...
    public async Task Handle_ShouldSetCorrectEventType()
    {
        ...
        // ASSERT
        capturedAuditLog!.EventType.Should().Be(AuditEventConstants.UserLocked);
    }

    [Fact]
    public async Task Handle_ShouldCaptureLockoutTimestamp()
    {
        // ARRANGE
        var applicationId = ApplicationId.Create(Guid.NewGuid());
        var email = Email.Create("user@example.com");
        var userId = EndUserId.Create(Guid.NewGuid());
        var lockedUntil = DateTime.UtcNow.AddHours(48);
        ...
        // ASSERT
        capturedAuditLog!.Details.Should().Contain("lockedUntil");
    }
}
