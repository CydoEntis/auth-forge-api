    ...
    : ICommandHandler<RefreshEndUserTokenCommand, Result<RefreshEndUserTokenResponse>>
{
    ...
    public RefreshEndUserTokenCommandHandler(
        ...
        ILogger<RefreshEndUserTokenCommandHandler> logger)
    {
    ...
    {
        _logger.LogDebug("Token refresh attempt from IP {IpAddress}", command.IpAddress);
        ...
        {
            _logger.LogWarning("Token refresh failed - Invalid refresh token from IP {IpAddress}", command.IpAddress);
        ...
        if (user is null)
        {
            _logger.LogError("Token refresh failed - User not found for UserId {UserId}", refreshToken.UserId);
        ...
        {
            _logger.LogError("Token refresh failed - Application not found for ApplicationId {ApplicationId}", user.ApplicationId);
        ...
        if (validationResult.IsFailure)
        {
            ...
            {
                _logger.LogWarning(
                    "⚠️ SECURITY ALERT: Token theft detected! Revoked token reused for user {UserId} ({Email}) from IP {IpAddress}. Revoking all user tokens.",
                    user.Id, user.Email.Value, command.IpAddress);
                await RevokeAllUserTokensAsync(user.Id, cancellationToken);
            }
            else
            {
                _logger.LogWarning("Token refresh failed for user {UserId} - {ErrorCode}: {ErrorMessage}",
                    user.Id, validationResult.Error.Code, validationResult.Error.Message);
            }
        ...
        _logger.LogInformation("Token successfully refreshed for user {UserId} ({Email}) from IP {IpAddress}",
            user.Id, user.Email.Value, command.IpAddress);
